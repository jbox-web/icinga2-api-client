...
- **Object actions are shared** via the `Actions` module (`lib/icinga2/api/actions.rb`), included by both `Host` and `Service`: `schedule_downtime`, `acknowledge`, `remove_acknowledgement`. Includers provide `action_type` (`"Host"`/`"Service"`), `action_filter` (the Icinga filter selecting the object), and `build_scheduled_downtime(name)`. `run_action` posts `/actions/<name>` and raises if nothing was affected. `schedule_downtime` returns a `Downtime`, not the raw payload.
